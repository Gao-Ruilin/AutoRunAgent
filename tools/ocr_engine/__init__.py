...
def load_from_hf_export(
    *,
    ...
    from .model import FalconPerception
...
def load_from_hf_export_mlx(
    *,
    ...
    Only available on Apple Silicon with the ``mlx`` package installed.
    """
    try:
        import mlx.core as mx
        from mlx.utils import tree_map
        from .mlx.convert import load_mlx_weights
        from .mlx.model import FalconPerception as FalconPerceptionMLX
    except ImportError as e:
        raise ImportError(
            "MLX backend is only available on Apple Silicon. "
            "Install the 'mlx' package or use backend='torch'."
        ) from e
    from huggingface_hub import snapshot_download
