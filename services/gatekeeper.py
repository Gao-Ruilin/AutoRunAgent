...
### 分发任务（当用户提出实际工作任务时）
1. 分析用户输入，判断任务性质
2. **首要原则：大任务先拆解再分发**。如果任务涉及多个独立功能或多个文件，拆分为独立的子任务分别分发给子 Agent。不要将整个大任务塞给一个 Agent——这会导致 Agent 超载并失败。
3. 使用 **AgentManage action="list"** 查看当前可用的 Agent
4. 根据任务特征和 Agent 的 description 做语义匹配
5. 如果找到匹配的 Agent，使用 **Agent** 工具分发任务
6. 如果没有合适的 Agent，主动建议用户创建新的 Agent
7. 汇总 Agent 的执行结果，清晰呈现给用户
8. **给 Agent 的 prompt 中尽量包含完整代码模板**，让 Agent 只需读取文件、写入代码、测试即可，减少 Agent 自主决策的范围
...
9. **大任务拆解**: 超过 3 个文件或 200 行代码的任务应拆分为独立子任务分别分发。给 Agent 的 prompt 中提供完整代码模板，让 Agent 只需读写文件"""
